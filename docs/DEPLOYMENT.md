...
This guide deploys the whole system step by step. Every command and setting below matches the files in this repository (`render.yaml`, `apps/web/vercel.json`, `apps/mobile/eas.json`, `apps/mobile/app.json`, `apps/api/src/env.ts`).

| Part | Platform | Config in repo |
|------|----------|----------------|
| Database + uploaded photos | MongoDB Atlas (replica set, GridFS) | `MONGODB_URI` |
| API (`apps/api`, Node.js + Express) | Render web service `bharatsales-ai` | `render.yaml` |
| Web dashboard (`apps/web`, React + Vite) | Vercel | `apps/web/vercel.json` |
...
| Android APK | GitHub release `v1.0.4-android` (build 5) | talks to the Render API | new build 1.0.5 (6) must be built with EAS on the same Expo project so it installs as an update |
...
2. **Render → Settings → Build & Deploy** — the build must build the shared packages first and the start command must run `apps/api/dist/main.js`:
   - Build: `corepack enable && pnpm install --frozen-lockfile --prod=false && pnpm --filter @bharatsales/api... build`
   - Start: `node apps/api/dist/main.js` (or `node dist/main.js` if the service's Root Directory is `apps/api`)
3. **Vercel → Settings**
   - Root Directory: `apps/web` (so `apps/web/vercel.json` is used; it sets framework Vite, install/build commands and `dist` output).
...
Checked against `apps/api/src/env.ts` (validated at boot) and every `process.env` read in `apps/api/src`:
...
| `STORAGE_DRIVER` | `gridfs` (in Blueprint) | Where uploaded photos go. `gridfs` stores them in MongoDB, where they survive deploys; this is also the default. `local` writes to `apps/api/uploads`, which Render wipes on every deploy. |
...
2. **Root Directory: `apps/web`**. Framework preset: **Vite**. `apps/web/vercel.json` overrides the rest, so leave the dashboard fields at their defaults:

   | Setting | Value (from `apps/web/vercel.json`) |
   |---------|--------------------------------------|
   | Framework | `vite` |
   | Install command | `cd ../.. && pnpm install --frozen-lockfile` (whole workspace) |
   | Build command | `cd ../.. && pnpm --filter "@bharatsales/web..." build` (builds workspace deps, then `tsc --noEmit && vite build`) |
   | Output directory | `dist` (that is, `apps/web/dist`) |
...
Then update the download link in `README.md`. The current published build is [v1.0.4 (build 5)](https://github.com/rajak73/bharatsales-ai/releases/download/v1.0.4-android/BharatSales-AI-v1.0.4.apk).
...
Never run the demo seed (`pnpm --filter @bharatsales/api seed`) against production. It **deletes** the data in the target collections and creates demo accounts that all share the password `password123`. Its only safety guard is `NODE_ENV=production`.
...
- The seed scripts do **not** load `apps/api/.env`. They read only the variables on the command line or in your shell.
...
- `.github/workflows/ci.yml` runs on every push and pull request to `main`: type-check, lint, seed smoke test, tests and build for all workspaces against a throwaway MongoDB replica set, plus a separate `apps/web` Vite build. **CI does not deploy.** Render and Vercel deploy from `main` on their own.
