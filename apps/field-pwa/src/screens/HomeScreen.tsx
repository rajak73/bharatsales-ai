import { Bell, Store, ChevronRight, AlertTriangle } from 'lucide-react';
...
export function HomeScreen() {
  ...
  const [userInitials] = useState(getUserInitials);
      ...
      <div className="bg-primary-600 px-5 pt-12 pb-4 flex items-center justify-between shadow-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-bold backdrop-blur-sm border border-white/30"
          >
            {userInitials}
          </button>
      ...
      <div className="px-5 py-6 space-y-6">
            ...
            <>
              <div className="flex justify-between items-end mb-2">
                <div>
                  ...
                  <p className="text-slate-800 font-bold">{formatCurrency(targetData.goal)}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-0.5">Achieved</p>
                  <p className="text-slate-800 font-bold">{formatCurrency(targetData.achieved)}</p>
                </div>
              </div>

              <div className="relative h-6 w-full bg-slate-200 rounded-full overflow-hidden mb-4 shadow-inner">
                <div
                  className="absolute top-0 left-0 h-full bg-primary-600 rounded-full transition-all duration-1000 flex items-center justify-end pr-2"
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <h2 className="text-slate-800 text-xl font-bold">Today's Beat</h2>
            <button onClick={() => navigate('/beat')} className="flex items-center gap-1 text-xs font-bold text-primary-600">
              ...
              {beatPreview.map((outlet) => (
                <div key={outlet.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="bg-primary-100 text-primary-600 font-bold text-xs px-3 py-2 rounded-lg hover:bg-primary-200 transition-colors shrink-0"
                  >
                    Visit
                  </button>
                </div>
